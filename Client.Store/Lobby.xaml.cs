...
namespace Client.Store
{
    ...
    public sealed partial class Lobby : Page
    {
        ...
        public Lobby()
        {
            ...
            this.navigationHelper.GoBackCommand = new Client.Store.Common.RelayCommand(() => this.GoBack(), () => this.CanGoBack());
            this.serverListView.SelectionChanged += serverListView_SelectionChanged;
            this.userListView.SelectionChanged += serverListView_SelectionChanged;
        ...
        private const int MinimumWidthForSupportingThreePanes = 1152;
        ...
        {
            return Window.Current.Bounds.Width < MinimumWidthForSupportingThreePanes;
        }

        private bool UsingOnePanle()
        {
            return Window.Current.Bounds.Width < MinimumWidthForSupportingTwoPanes;
        }

        private bool UsingTwoPanels()
        {
            return Window.Current.Bounds.Width < MinimumWidthForSupportingThreePanes && Window.Current.Bounds.Width >= MinimumWidthForSupportingTwoPanes;
        }
        ...
        {
            if (this.UsingLogicalPageNavigation() && this.serverListView.SelectedItem != null)
            {
                return true;
            }
            else
        ...
        private void GoBack()
        {
            if (this.UsingLogicalPageNavigation() && this.userListView.SelectedItem != null)
            {
                ...
                // Rückwärtsnavigation.
                this.userListView.SelectedItem = null;
            }
            else if (this.UsingLogicalPageNavigation() && this.serverListView.SelectedItem != null)
            {
                // Wenn die logische Seitennavigation wirksam ist und ein ausgewähltes Element vorliegt, werden die
                // Details dieses Elements aktuell angezeigt.  Beim Aufheben der Auswahl wird die
                // Elementliste wieder aufgerufen.  Aus Sicht des Benutzers ist dies eine logische Rückwärtsnavigation
                // Rückwärtsnavigation.
                this.serverListView.SelectedItem = null;
            }
            else
            {
        ...
        private string DetermineVisualState()
        {
            ...
            if (this.UsingOnePanle())
            {
                if (this.serverListView.SelectedItem == null)
                    return "SinglePane";
                else if (this.userListView.SelectedItem == null)
                    return "SinglePane_Users";
                return "SinglePane_Detail";
            }
            else if (this.UsingTwoPanels())
            {
                if (this.serverListView.SelectedItem == null)
                    return "DuoPane_Servers";
                return "DuoPane_Users";
            }

            throw new NotSupportedException();
        }

        #endregion Logische Seitennavigation
